...
export default function Organizations() {
  ...
  const { organizationTypes, organizationTypeLabel } = useDomainLabels();
    ...
    <MainLayout>
      <div className="bg-muted/30 border-b">
        <div className="container mx-auto px-4 py-12">
          <h1 className="text-4xl font-bold mb-6">{t("organizations.title")}</h1>
          ...
          <div className="flex flex-col md:flex-row gap-4 max-w-3xl">
            ...
            <Select value={type} onValueChange={setType}>
              <SelectTrigger className="w-full md:w-[250px] h-12 bg-background">
                <SelectValue placeholder={t("organizations.typePlaceholder")} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{t("organizations.allTypes")}</SelectItem>
      ...
      <div className="container mx-auto px-4 py-12">
        {isLoading ? (
          <div className="text-center py-20 text-muted-foreground">{t("organizations.loading")}</div>
        ) : organizations?.length === 0 ? (
          <div className="text-center py-20">
            <h3 className="text-xl font-semibold mb-2">{t("organizations.emptyTitle")}</h3>
            <p className="text-muted-foreground">{t("organizations.emptyDescription")}</p>
          </div>
        ) : (
                ...
                <CardContent className="p-6">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <Badge variant="secondary" className="bg-primary/5 text-primary">
                      {organizationTypeLabel(org.type) || org.type}
                    </Badge>
                  </div>
                  ...
                  {org.website && (
                    <a href={org.website.startsWith('http') ? org.website : `https://${org.website}`} target="_blank" rel="noopener noreferrer">
                      <Button variant="outline" className="w-full text-sm h-9">
                        {t("organizations.visitWebsite")} <ExternalLink className="ml-2 h-3 w-3" />
                      </Button>
                    </a>
                  )}
                </CardContent>
