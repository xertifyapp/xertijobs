...
- Empresa registration creates the org with status `pendiente` + a user account; login returns 403 `pendiente_aprobacion` until the org is `verificada` by admin in /admin (suspended orgs return `org_suspendida`, rejected orgs return `org_rechazada`). Old /registro-organizacion route redirects to /registro?tipo=empresa.
...
- Organization verification is admin-only. Statuses: `pendiente` (just registered), `verificada` (approved by SEMBER, visible publicly + can publish opportunities), `suspendida` (previously verified, activity suspended), `rechazada` (application rejected). Only admins change the status via `PATCH /organizations/:id` in /admin; the server stamps `verifiedBy` (admin user id) + `verifiedAt` (timestamptz) on every admin status change. Non-verified orgs are hidden from the public directory and their opportunities are hidden from public list/detail (owner empresa + admin exempt). Legacy value `aprobada` was migrated to `verificada`.
- Admin opportunity moderation (/admin, "Moderación de Oportunidades" section): admins see ALL opportunities across every org (list endpoint has no publication restriction for admins) and can aprobar (→activa), pausar (→pausada), cerrar (→cerrada), destacar/quitar destacado (`featured` boolean), editar (shared `OpportunityForm` dialog with an org selector, also used to crear on behalf of any org), and eliminar (AlertDialog confirm). Section has a text search (title/org name) + status filter. `featured` is a SEMBER-only field: the `PATCH /opportunities/:id` handler strips `featured` from non-admin payloads; if stripping leaves an empty update it returns the current row unchanged (Drizzle throws "No values to set" on an empty `.set()`). New opportunity status `pausada` added (statuses: activa | pausada | cerrada | borrador). All opp mutations invalidate list + global-stats + recent-activity query keys.
- Admin org CRUD lifecycle (/admin): admins can create, edit, and delete organizations plus approve/reject/suspend/classify them. `POST /organizations` is admin-only (`requireRole("admin")`) — public empresa self-registration goes through `/auth/register` (always `pendiente`), NOT this endpoint. On create, admins may set any status; if status != `pendiente` the server stamps `verifiedBy`/`verifiedAt`. UI uses the shared `OrganizationForm` dialog (create + edit) in `Admin.tsx`; delete uses an AlertDialog confirm. All mutations invalidate list/pending/global-stats/recent-activity query keys. Org fields include `contactEmail`, `contactPhone`, `verificationDocs` (link to supporting docs).
- Domain values are plain-text Spanish enums: org types (universidad, empresa, gobierno, ong, fundacion, organismo_internacional, academia, bootcamp), opportunity types (internship, empleo, beca, bootcamp, evento, movilidad, convocatoria), modality (presencial, remoto, hibrido), application status (enviada, en_revision, preseleccionado, aceptado, rechazado), opportunity status (activa, pausada, cerrada, borrador).
- Language preference is persisted per user (`users.preferredLanguage`, es/en/pt, default es) so it survives device/browser changes. Returned on `AuthUser` (login + /auth/me); updated via `PATCH /auth/me`. Frontend `LanguageSync` (mounted in App) applies the stored preference on load — server preference wins over localStorage; `LanguageSwitcher` persists a signed-in user's pick (optimistic cache update + mutation). Registration captures the current UI language. OTP/verification emails use the user's stored language (register falls back to request locale for brand-new accounts).
